...
- `-o <dir>` or `--out <dir>`: sets the directory of the output files. Defaults to `./quarkdown-output`.
...
- `--log-level <level>` (placed before the subcommand, e.g. `quarkdown --log-level debug c file.qd`): sets the minimum log level. Accepted values are `debug`, `info`, `warn`, `error`, and `none`. Defaults to `info`. It can also be set via the `QD_LOG_LEVEL` environment variable, with the option taking precedence. If set to `warn` or higher, the output content is not printed out.