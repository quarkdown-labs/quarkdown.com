...
Several built-in nodes, such as [headings](headings.md), are *primitive function-backed*: the stdlib exposes functions whose parameters mirror the node’s properties. Whenever an `.extend` wrapper is registered for that name, every occurrence of the primitive must be routed through the wrapper.
