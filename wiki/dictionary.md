...
> **Example 1**
> 
> [`.foreach`](loops.qd) accepts both iterables and dictionaries, so we explicitly create a dictionary to iterate over key-value pairs, and [destructure](destructuring.qd) them into separate variables.
> 
> ```markdown
> .var {mydictionary}
>     .dictionary
...
You can nest dictionaries when the function supports it, such as [`.localization`](localization.qd):
...
You can pass a dictionary to any function that accepts an [iterable](iterable.qd), where it is treated as an iterable of pairs.
